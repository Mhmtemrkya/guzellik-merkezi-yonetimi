...
public interface IPendingOperationService
{
    ...
    Task<Result<PendingOperationDto>> GetAsync(Guid tenantId, Guid id, Guid actorUserId, UserRole? actorRole, CancellationToken cancellationToken = default);
    ...
    Task<Result> CancelAsync(Guid tenantId, Guid id, Guid decidedByUserId, UserRole? actorRole, CancellationToken cancellationToken = default);
}
