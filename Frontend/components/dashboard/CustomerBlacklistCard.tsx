...
export default function CustomerBlacklistCard({
  ...
  return (
    <div className="rounded-[16px] border border-[#EAD8DF] bg-white/70 p-3">
      {!open ? (
        ...
        <div className="space-y-2">
          ...
          <input value={reasonText} onChange={(e) => setReasonText(e.target.value)} placeholder="Sebep (opsiyonel) — ör. tekrar no-show"
            className="w-full rounded-lg border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] text-[#2A2027] outline-none focus:border-rose-300" />
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => { setOpen(false); setReasonText('') }}
              className="inline-flex items-center gap-1 rounded-lg border border-[#EAD8DF] bg-white px-2.5 py-1 text-[11.5px] text-[#5A4B53]"><X className="h-3 w-3" /> Vazgeç</button>
