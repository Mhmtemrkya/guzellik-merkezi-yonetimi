...
export default function CustomerSalesPanel({
  ...
  return (
    <>
      <div className="rounded-[18px] border border-[#f0e0e6] bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#f7ebef] pb-3">
          ...
          {canManage && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-[10px] border border-[#efe1e7] bg-[#fffafc] px-2.5 py-1 text-[10px] font-semibold text-[#a34a62] transition-colors hover:border-[#e7bccb] hover:bg-[#fff2f6]"
            >
        ...
        <div className="mt-3 space-y-2">
          {visible.length === 0 ? (
            <div className="rounded-[12px] border border-dashed border-[#ead8df] bg-[#fffafb] px-3 py-6 text-center text-[11px] text-[#705a66]">
              {sorted.length === 0
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex flex-wrap items-center gap-1.5">
          <span className="truncate text-[12px] font-semibold text-[#352432]">{account.name}</span>
          <span className={`rounded-full border px-1.5 py-0.5 text-[9px] font-bold ${meta.pill}`}>{meta.label}</span>
          {account.isHistorical && (
            <span className="inline-flex items-center gap-0.5 rounded-full border border-[#e0d3f2] bg-[#faf6ff] px-1.5 py-0.5 text-[9px] font-bold text-[#6b4aa0]">
              <Archive className="h-2.5 w-2.5" /> Geçmiş kayıt
            </span>
          )}
        </span>

        <span className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[10px] text-[#705a66]">
          <span className="inline-flex items-center gap-1"><CalendarClock className="h-2.5 w-2.5 text-[#c85776]" /> {formatSaleDate(account.soldAtUtc)}</span>
          {account.soldByStaffName && (
            <span className="inline-flex items-center gap-1"><User className="h-2.5 w-2.5 text-[#c85776]" /> {account.soldByStaffName}</span>
          )}
        ...
        {account.saleStatus === 'Cancelled' && account.cancellationReason && (
          <span className="mt-1 block truncate text-[10px] italic text-[#b3453f]">Gerekçe: {account.cancellationReason}</span>
        )}
      ...
      <span className="flex shrink-0 flex-col items-end gap-1">
        <span className="font-display text-[13px] font-bold text-[#352432]">{formatTL(Math.round(account.totalAmount))}</span>
