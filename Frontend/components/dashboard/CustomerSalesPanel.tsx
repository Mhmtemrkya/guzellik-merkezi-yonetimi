...
export default function CustomerSalesPanel({
  ...
  const historyButton = canManage ? (
    <button
      ...
      className={`inline-flex shrink-0 cursor-pointer items-center gap-1.5 rounded-[10px] border border-[#EAD8DF] bg-[#F7F6F6] font-semibold text-[#a34a62] transition-colors hover:border-[#e7bccb] hover:bg-[#fff2f6] ${
    ...
    <>
      <div className={flush ? '' : 'rounded-[18px] border border-[#f0e0e6] bg-white p-4'}>
        ...
        {!flush && (
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#f7ebef] pb-3">
            <div className="flex items-center gap-2">
              <Package className="h-3.5 w-3.5 text-[#A5556E]" />
              <span className="text-[11px] font-semibold uppercase tracking-wider text-[#A5556E]">Paket & Hizmet Satışları</span>
              {sorted.length > 0 && (
              ...
              {sorted.length > 0 && FILTERS.filter((f) => f.key === 'all' || counts[f.key] > 0).map((f) => (
                <button
                  ...
                  className={`cursor-pointer rounded-full font-semibold transition-colors ${flush ? 'px-3 py-1.5 text-[11px]' : 'px-2.5 py-1 text-[10px]'} ${
                    filter === f.key ? 'bg-[#A5556E] text-white' : 'border border-[#f0e0e6] bg-white text-[#74616A] hover:border-[#e7bccb] hover:text-[#a34a62]'
                  }`}
                >
        ...
        <div className={`mt-3 ${flush ? 'grid gap-2.5 xl:grid-cols-2' : 'space-y-2'}`}>
          {visible.length === 0 ? (
            <div className={`rounded-[12px] border border-dashed border-[#EAD8DF] bg-[#F7F6F6] px-3 text-center text-[12px] text-[#74616A] ${flush ? 'py-10 xl:col-span-2' : 'py-6'}`}>
              {sorted.length === 0
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex flex-wrap items-center gap-1.5">
          <span className="truncate text-[12.5px] font-semibold text-[#2A2027]">{account.name}</span>
        ...
        <span className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] text-[#74616A]">
          <span className="inline-flex items-center gap-1"><CalendarClock className="h-3 w-3 text-[#A5556E]" /> {formatSaleDate(account.soldAtUtc)}</span>
          {account.soldByStaffName && (
            <span className="inline-flex items-center gap-1"><User className="h-3 w-3 text-[#A5556E]" /> {account.soldByStaffName}</span>
          )}
          {account.appliedByStaffName && (
            <span className="inline-flex items-center gap-1" title="Seansları uygulayan"><Sparkles className="h-3 w-3 text-[#A5556E]" /> {account.appliedByStaffName}</span>
          )}
          ...
          {account.sessionsTotal > 0 && (
            <span className="font-semibold tabular-nums text-[#3E343A]">
              ...
              <span className="ml-1 font-normal text-[#74616A]">({account.sessionsTotal} seanslık)</span>
            </span>
          )}
        </span>
        ...
        <span className="mt-1.5 flex items-center gap-2">
          ...
          <span className="shrink-0 text-[11px] font-semibold text-[#74616A]">
      ...
      <span className="flex shrink-0 flex-col items-end gap-1">
        <span className="font-display text-[13.5px] font-bold text-[#2A2027]">{formatTL(Math.round(account.totalAmount))}</span>
        <ChevronRight className="h-3.5 w-3.5 text-[#c9b3bd] transition-transform group-hover:translate-x-0.5 group-hover:text-[#A5556E]" />
      </span>
    </motion.button>
  )
}
