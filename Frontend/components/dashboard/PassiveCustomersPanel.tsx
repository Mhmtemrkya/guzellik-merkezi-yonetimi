...
export default function PassiveCustomersPanel({ tenantId, onSelect }: { tenantId?: string; onSelect?: (id: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      {/* Eşik ayarı */}
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-[18px] border border-[#EAD8DF] bg-white/80 p-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-[13px] font-semibold text-[#2A2027]">Pasif müşteri eşiği</div>
            <div className="text-[11px] text-[#74616A]">Bu kadar gündür randevu/paket işlemi olmayan müşteriler pasif sayılır</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <input type="number" min={1} value={days} onChange={(e) => setDays(Number(e.target.value))}
            className="w-20 rounded-lg border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-center text-[13px] font-semibold text-[#2A2027] outline-none focus:border-[#A5556E]" />
          <span className="text-[12px] text-[#5A4B53]">gün</span>
          <button type="button" disabled={busy} onClick={save}
            className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-[#A5556E] to-[#8e3f5b] px-3.5 py-1.5 text-[12px] font-semibold text-white transition disabled:opacity-50">
      ...
      {/* Liste */}
      {loading ? (
        <div className="py-8 text-center text-[12px] text-[#74616A]">Yükleniyor…</div>
      ) : items.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-10 text-center">
          <Moon className="h-7 w-7 text-[#cdb8c1]" />
          <div className="text-[13px] font-medium text-[#5A4B53]">Pasif müşteri yok</div>
          <div className="text-[11px] text-[#74616A]">Son {data?.thresholdDays} gün içinde tüm müşterilerin işlemi var.</div>
        </div>
      ) : (
        <>
          <div className="flex items-center gap-2 px-1 text-[11px] text-[#5A4B53]">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {items.map((p) => (
              <button key={p.id} type="button" onClick={() => p.id && onSelect?.(p.id)}
                className="group flex items-center justify-between gap-3 rounded-[14px] border border-[#f0e0e6] bg-[#F7F6F6] px-3 py-2.5 text-left transition hover:border-[#BE7690] hover:bg-[#F6DFE6]">
                <div className="min-w-0">
                  <div className="truncate text-[13px] font-medium text-[#2A2027]">{p.fullName}</div>
                  <div className="text-[11px] text-[#74616A]">{p.phone}</div>
                </div>
