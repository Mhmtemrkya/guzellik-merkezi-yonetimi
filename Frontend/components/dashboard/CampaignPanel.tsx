...
export default function CampaignPanel({ tenantId }: { tenantId?: string }) {
  ...
  return (
    <div id="kampanyalar" className="rounded-[22px] border border-[#EAD8DF] bg-white p-5 shadow-[0_18px_42px_-34px_rgba(150,78,104,0.42)]">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          className="inline-flex items-center gap-1.5 rounded-full bg-[#A5556E] px-3 py-1.5 text-[11px] font-medium text-white transition-opacity hover:opacity-90"
        >
      ...
      {open && (
        <div className="mb-4 grid gap-2.5 rounded-[16px] border border-[#f0e0e6] bg-[#F7F6F6] p-3 sm:grid-cols-2">
          <input
            value={form.name}
            onChange={(e) => setForm({ ...form, name: e.target.value })}
            placeholder="Kampanya adı (örn. Yaz İndirimi)"
            className="col-span-full rounded-[10px] border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] outline-none focus:border-[#A5556E]"
          />
          <select value={form.discountType} onChange={(e) => setForm({ ...form, discountType: e.target.value as DiscountTypeKey })} className="rounded-[10px] border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] outline-none focus:border-[#A5556E]">
          ...
          <input
            ...
            className="rounded-[10px] border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] outline-none focus:border-[#A5556E]"
          />
          <select value={form.target} onChange={(e) => setForm({ ...form, target: e.target.value as CampaignTargetKey, targetId: '' })} className="rounded-[10px] border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] outline-none focus:border-[#A5556E]">
            {(Object.keys(TARGET_LABELS) as CampaignTargetKey[]).map((t) => <option key={t} value={t}>{TARGET_LABELS[t]}</option>)}
          </select>
          {form.target !== 'All' ? (
            <select value={form.targetId} onChange={(e) => setForm({ ...form, targetId: e.target.value })} className="rounded-[10px] border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] outline-none focus:border-[#A5556E]">
          ...
          <label className="flex flex-col text-[9px] font-mono uppercase tracking-wide text-[#74616A]">
            Başlangıç
            <input type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} className="mt-0.5 rounded-[10px] border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] text-[#2A2027] outline-none focus:border-[#A5556E]" />
          </label>
          <label className="flex flex-col text-[9px] font-mono uppercase tracking-wide text-[#74616A]">
            Bitiş
            <input type="date" value={form.endDate} onChange={(e) => setForm({ ...form, endDate: e.target.value })} className="mt-0.5 rounded-[10px] border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] text-[#2A2027] outline-none focus:border-[#A5556E]" />
          </label>
        ...
        {campaigns.map((c) => (
          <div key={c.id} className="flex items-center justify-between gap-3 rounded-[14px] border border-[#f0e0e6] bg-white px-3 py-2.5">
            <div className="flex min-w-0 items-center gap-2.5">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-[10px] bg-[#A5556E] text-white">
                <Tag className="h-4 w-4" />
              </span>
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="truncate text-[13px] font-medium text-[#2A2027]">{c.name}</span>
                  {c.isRunning ? (
                    ...
                    <span className="rounded-full border border-[#EAD8DF] bg-[#F7F6F6]/60 px-1.5 py-0.5 text-[8px] font-mono uppercase tracking-wide text-[#74616A]">Pasif</span>
                  )}
                </div>
                <div className="truncate text-[10px] font-mono text-[#74616A]">
                  {c.discountType === 'Percent' ? `%${c.discountValue}` : formatTL(c.discountValue)} · {targetName(c.target, c.targetId)} · {c.startDate} → {c.endDate}
                </div>
              </div>
            </div>
            <button type="button" disabled={busy} onClick={() => remove(c.id)} className="text-[#74616A] transition-colors hover:text-rose-600 disabled:opacity-40" aria-label="Kampanyayı sil">
