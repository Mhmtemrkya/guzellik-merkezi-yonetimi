...
import { Ban, Search, XCircle } from 'lucide-react'
import type { CustomerAccount } from '@/lib/types'
import { formatTL } from '@/lib/apiMappers'

// İptal edilen satışlar ana listeyi kirletmesin diye ayrı modalde toplanır:
// borç/vade akışının dışındadırlar (tahsil edilmezler) ama kayıt olarak durur.
...
export default function CancelledSalesModal({ accounts, open, onOpenChange, onSelect }: Props) {
  const [query, setQuery] = useState('')

  const list = useMemo(() => {
    const q = query.trim().toLocaleLowerCase('tr')
    const cancelled = accounts.filter((a) => a.saleStatus === 'Cancelled')
    const filtered = q
      ? cancelled.filter((a) => `${a.customerName} ${a.name} ${a.servicePackageName}`.toLocaleLowerCase('tr').includes(q))
      : cancelled
  ...
  const totalPaid = list.reduce((s, a) => s + a.paidAmount, 0)
        ...
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto px-5 py-4">
          {list.map((a) => (
            <button
              ...
              className="w-full rounded-[14px] border border-[#f0dae2] bg-white px-3.5 py-3 text-left transition-colors hover:border-rose-200 hover:bg-rose-50/40"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="truncate text-[13.5px] font-semibold text-[#352432]">{a.customerName || a.name}</div>
                  <div className="truncate text-[11px] text-[#705a66]">{a.servicePackageName || a.name}</div>
                </div>
                <div className="shrink-0 text-right">
                  <div className="font-display text-[15px] tabular-nums text-[#352432]">{formatTL(a.totalAmount)}</div>
                  <div className="text-[10px] text-[#705a66]">tahsil {formatTL(a.paidAmount)}</div>
                </div>
              </div>
