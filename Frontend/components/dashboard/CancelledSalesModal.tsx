    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div className="shrink-0 border-b border-[#f2e2e9] bg-gradient-to-r from-[#fff5f6] via-white to-[#fff2f4] px-5 py-4">
          ...
          <div className="mt-3 inline-flex items-center rounded-full border border-[#EAD8DF] bg-white p-0.5">
            {([
              ...
              >
                {t.label}
                <span className={`ml-1.5 tabular-nums ${tab === t.key ? 'text-[#7a2f4a]' : 'text-[#74616A]'}`}>{t.count}</span>
              </button>
            ))}
          </div>
        ...
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto px-5 py-4">
          {list.map((a) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="truncate text-[13.5px] font-semibold text-[#2A2027]">{a.customerName || a.name}</div>
                  <div className="truncate text-[11px] text-[#74616A]">{a.name}</div>
                </div>
                <div className="shrink-0 text-right">
                  ...
                  {isRefundTab ? (
                    <>
                      ...
                      <div className="text-[10px] text-[#74616A]">tahsil {formatTL(a.collectedAmount)}</div>
                      <div className="text-[10px] text-emerald-700">kurumda {formatTL(a.retainedAmount)}</div>
                    </>
                  ) : (
                    <>
                      <div className="font-display text-[15px] tabular-nums text-[#2A2027]">{formatTL(a.totalAmount)}</div>
                      <div className="text-[10px] text-[#74616A]">tahsil {formatTL(a.collectedAmount)}</div>
          ...
          {list.length === 0 && (
            <div className="rounded-[14px] border border-dashed border-[#EAD8DF] bg-[#F7F6F6] px-4 py-10 text-center text-[12px] text-[#74616A]">
              {isRefundTab
