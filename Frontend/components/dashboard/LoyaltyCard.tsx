...
export default function LoyaltyCard({ customerId, tenantId }: { customerId?: string; tenantId?: string }) {
  ...
  return (
    <div className="rounded-[20px] border border-[#EAD8DF] bg-gradient-to-br from-white/90 to-[#fff1f6]/70 p-4 shadow-[0_18px_50px_-40px_rgba(142,63,91,0.5)]">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-mono text-[#74616A]">puan</span>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        <input
          ...
          className="w-20 rounded-[10px] border border-[#EAD8DF] bg-white px-2.5 py-1.5 text-[12px] outline-none focus:border-[#A5556E]"
        />
        ...
        <div className="mt-3 space-y-1">
          {history.map((h, i) => (
            <div key={h.id || i} className="flex items-center justify-between rounded-[10px] border border-[#f0e0e6] bg-white/70 px-2.5 py-1.5 text-[11px]">
              <span className="truncate text-[#5A4B53]">{h.description || h.sourceType}</span>
