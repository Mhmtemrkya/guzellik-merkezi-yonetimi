...
export default function FeatureLockedCard({ title, message }: { title: string; message?: string }) {
  return (
    <div className="rounded-[20px] border border-dashed border-[#e6c9d4] bg-gradient-to-br from-[#fffafb] to-[#fff1f6]/50 p-4">
      <div className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-[#A5556E]/60">
        <Lock className="h-3.5 w-3.5" /> {title}
      </div>
      <div className="mt-2 text-[12px] leading-5 text-[#5A4B53]">
        {message ?? 'Bu özellik mevcut paketinizde yok.'}
      </div>
      <div className="mt-2 inline-flex items-center gap-1 rounded-lg border border-[#BE7690] bg-white/70 px-2 py-1 text-[10.5px] font-semibold text-[#8C4460]">
