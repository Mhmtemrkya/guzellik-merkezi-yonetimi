...
<ModalPortal>
    <AnimatePresence>
      {open && (
        <motion.div className="fixed inset-0 z-[145] grid place-items-center bg-white/80 px-4 backdrop-blur-sm"
          ...
          <motion.div initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: 0.98 }} transition={{ duration: 0.22 }}
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-[12px] bg-[#A5556E] text-white">
                  <MonitorSmartphone className="h-5 w-5" strokeWidth={1.6} />
                </span>
                <div>
                  <div className="font-display text-xl tracking-tight text-[#2A2027]">Cihaz Güvenliği</div>
                  <div className="text-[11px] text-[#5A4B53]">{staffName} · tanımlı cihazlar ve giriş kısıtı</div>
                </div>
              </div>
              <button type="button" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-[9px] border border-[#EAD8DF] text-[#5A4B53] hover:bg-[#F7F6F6]/50">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between gap-3 rounded-[14px] border border-[#EAD8DF] bg-[#F7F6F6] px-4 py-3">
              <div className="flex items-center gap-2.5">
                <ShieldCheck className={`h-4.5 w-4.5 ${settings?.enabled ? 'text-emerald-600' : 'text-[#74616A]'}`} />
                <div>
                  <div className="text-[12.5px] font-medium text-[#2A2027]">Cihaz güvenliği (kurum geneli)</div>
                  <div className="text-[10.5px] text-[#5A4B53]">
            ...
            {/* Cihaz limiti */}
            <div className="mt-3 flex flex-wrap items-end justify-between gap-3 rounded-[14px] border border-[#EAD8DF] bg-[#F7F6F6] px-4 py-3">
              <div>
                <div className="text-[12.5px] font-medium text-[#2A2027]">Cihaz limiti</div>
                <div className="text-[10.5px] text-[#5A4B53]">
              ...
              <div className="flex items-center gap-2">
                <input type="number" min={1} max={10} value={limitInput} placeholder="—"
                  ...
                  className="w-20 rounded-[10px] border border-[#EAD8DF]/80 bg-white px-3 py-2 text-center text-[13px] text-[#2A2027] outline-none focus:border-[#A5556E]" />
                <button type="button" onClick={saveLimit} disabled={saving}
                  className="rounded-[10px] bg-[#A5556E] px-3.5 py-2 text-[11px] font-medium text-white hover:opacity-90 disabled:opacity-50">
                  Kaydet
                </button>
              </div>
            </div>
            ...
            {/* Cihaz listesi */}
            <div className="mt-4">
              <div className="text-[10px] font-mono uppercase tracking-widest text-[#A5556E]/75">Tanımlı Cihazlar</div>
              <div className="mt-2 space-y-2">
                {loading && <div className="py-6 text-center text-[12px] text-[#74616A]">Yükleniyor…</div>}
                {!loading && devices.length === 0 && (
                  <div className="rounded-[12px] border border-dashed border-[#EAD8DF] px-4 py-6 text-center text-[12px] text-[#5A4B53]">
                ...
                {devices.map((d) => {
                  ...
                  return (
                    <div key={d.id} className="flex items-center gap-3 rounded-[14px] border border-[#EAD8DF] bg-white px-4 py-3">
                        ...
                        {renamingId === d.id ? (
                          <div className="flex items-center gap-2">
                            <input autoFocus value={renameValue} onChange={(e) => setRenameValue(e.target.value)}
                              ...
                              className="w-full max-w-[220px] rounded-[8px] border border-[#8C4460]/50 bg-white px-2 py-1 text-[12px] outline-none" />
                            <button type="button" onClick={() => void saveRename(d)} className="rounded-[8px] bg-[#A5556E] px-2 py-1 text-[10px] text-white">Tamam</button>
                          </div>
                        ) : (
                          <div className="truncate text-[12.5px] font-medium text-[#2A2027]">{d.name}</div>
                        )}
                        <div className="mt-0.5 flex flex-wrap items-center gap-x-2 truncate text-[9.5px] font-mono text-[#74616A]">
                      ...
                      <button type="button" title="Yeniden adlandır" onClick={() => { setRenamingId(d.id); setRenameValue(d.name) }}
                        className="grid h-8 w-8 shrink-0 place-items-center rounded-[9px] border border-[#EAD8DF] text-[#5A4B53] hover:bg-[#F7F6F6]/50">
