 ...
 * tüm hizmet + paketler ortak kategorilere göre gruplanır; kategoriye tıklayınca
 * o kategorinin hizmetleri ve paketleri ayrı bölümlerde listelenir, oradan satış yapılabilir.
 */
export default function CategoryExplorer({
  ...
  // Kategori havuzu: özel kategoriler + hizmet/paketlerde geçen tüm kategori adları + Kategorisiz
  ...
  }, [customCats, services, packages])

  const activeCat = selectedCat && categories.some((c) => c.name === selectedCat) ? selectedCat : categories[0]?.name || null
  const catServices = useMemo(() => services.filter((s) => (s.group || UNCATEGORIZED) === activeCat), [services, activeCat])
  const catPackages = useMemo(() => packages.filter((p) => (p.category || UNCATEGORIZED) === activeCat), [packages, activeCat])
    ...
    <>
      ...
      <div className="relative space-y-5 p-4 sm:p-6 lg:p-8">
            ...
            >
              ...
              <div className="overflow-hidden rounded-[18px] border border-[#ead8df]/70 bg-white/90">
                <div className="flex items-center justify-between border-b border-[#ead8df]/70 px-5 py-4">
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-widest text-[#c85776]/75">{activeCat} · Hizmetler</div>
                ...
                <div className="divide-y divide-[#f1e5ea]">
                  ...
                  {catServices.length === 0 && <div className="px-5 py-10 text-center text-[12px] text-[#352432]/45">Bu kategoride hizmet yok.</div>}
                </div>
              </div>
              ...
              <div className="overflow-hidden rounded-[18px] border border-[#ead8df]/70 bg-white/90">
                <div className="flex items-center justify-between border-b border-[#ead8df]/70 px-5 py-4">
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-widest text-violet-500/80">{activeCat} · Paketler</div>
                ...
                <div className="divide-y divide-[#f1e5ea]">
                  ...
                  {catPackages.length === 0 && <div className="px-5 py-10 text-center text-[12px] text-[#352432]/45">Bu kategoride paket yok.</div>}
                </div>
              </div>
            </motion.div>
          )}
        ...
        <div className="flex items-center gap-2 rounded-[12px] border border-[#efbfd0]/60 bg-[#fff1f6]/60 px-4 py-2.5 text-[11px] text-[#b14d6c]">
          <Tag className="h-4 w-4" /> Kategoriler hizmet ve paketlerde ortaktır — hizmet/paket formlarında bu kategorilerden seçim yapılır. <Plus className="h-3 w-3" /> ile yeni kategori herkese açılır.
        </div>
      </div>
    </>
  )
