    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          {/* Müşteri */}
          <div>
            <span className={label}><User className="h-3.5 w-3.5 text-[#c05277]" /> Müşteri</span>
            {customer ? (
              <div className="flex items-center justify-between gap-2 rounded-[12px] border border-[#c85776]/40 bg-[#fff1f6] px-3 py-2.5">
                <span className="flex min-w-0 items-center gap-2">
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white text-[11px] font-bold text-[#a3576f]">
                    {customer.name.trim().split(/\s+/).slice(0, 2).map((w) => w[0] || '').join('').toLocaleUpperCase('tr')}
                  </span>
                  <span className="truncate text-[13px] font-semibold text-[#352432]">{customer.name}</span>
                </span>
                <button type="button" onClick={() => setCustomer(null)} className="shrink-0 text-[11px] font-semibold text-[#a3576f] hover:underline">
                  Değiştir
                </button>
              </div>
            ) : (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className={label}><Package className="h-3.5 w-3.5 text-[#c05277]" /> Paket (opsiyonel)</span>
            ...
            <label className="block">
              <span className={label}><FileText className="h-3.5 w-3.5 text-[#c05277]" /> Cari adı</span>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className={label}><Wallet className="h-3.5 w-3.5 text-[#c05277]" /> Toplam tutar</span>
            ...
            <label className="block">
              <span className={label}><Banknote className="h-3.5 w-3.5 text-[#c05277]" /> Peşinat</span>
              <div className="flex items-center gap-1.5 rounded-[12px] border border-[#ead8df] bg-white px-3 py-2.5 focus-within:border-[#c85776]">
                <span className="text-[13px] font-semibold text-[#705a66]">₺</span>
                <input
          ...
          {/* Taksit planı */}
          <div className="rounded-[16px] border border-[#f0d9e2] bg-[#fffafc] p-3.5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="text-[11px] text-[#705a66]">
                Finanse edilen <b className="text-[#4a3a44]">{formatTL(financed)}</b>
              </span>
            </div>
            ...
            <label className="mt-3 block sm:max-w-[220px]">
              <span className={label}><CalendarDays className="h-3.5 w-3.5 text-[#c05277]" /> İlk vade</span>
              <input type="date" value={firstDue} onChange={(e) => setFirstDue(e.target.value)} className={field} />
            </label>

            {financed > 0 ? (
              <div className="mt-3">
                <div className="text-[11.5px] font-semibold text-[#4a3a44]">
                  {count} × {formatTL(perInstallment)}
                  <span className="ml-1.5 font-normal text-[#705a66]">· ilk vade {shortDay(firstDue)}</span>
                </div>
                <div className="mt-1.5 flex flex-wrap gap-1.5">
                  {preview.map((p) => (
                    <span key={p.no} className="rounded-[9px] border border-[#ead8df] bg-white px-2 py-1 text-[10.5px] text-[#4a3a44]">
                      #{p.no} · {shortDay(p.due)} · <b>{formatTL(p.amount)}</b>
                    </span>
                  ))}
                  {count > 4 && (
                    <span className="rounded-[9px] bg-[#fff1f6] px-2 py-1 text-[10.5px] font-semibold text-[#a3576f]">+{count - 4} taksit daha</span>
                  )}
                </div>
              </div>
            ) : (
              <div className="mt-3 rounded-[10px] border border-dashed border-[#ead8df] bg-white px-3 py-2 text-[11px] text-[#705a66]">
          ...
          <label className="block">
            <span className={label}><FileText className="h-3.5 w-3.5 text-[#c05277]" /> Not (opsiyonel)</span>
        ...
        <div className="shrink-0 border-t border-[#f2e2e9] bg-white px-5 py-3">
          <div className="flex items-center justify-between gap-3">
            <div className="text-[11px] text-[#705a66]">
              Toplam <b className="text-[#352432]">{formatTL(totalNum)}</b>
