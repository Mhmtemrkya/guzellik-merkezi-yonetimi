...
export function normalizeCustomer(customer: ApiCustomer | null | undefined, index = 0): Customer {
  ...
  return {
    ...
    activePackages: 0,
    // Liste uçlarında sunucudan gelir (sayfa satırı için hesaplanır); tekil uçta 0 kalır.
    totalSpent: Number(customer?.totalSpent ?? 0),
    debt: Number(customer?.debt ?? 0),
    remainingSessions: 0,
    lastVisit: 'Randevu API’sinden hesaplanacak',
    lastVisitUtc: customer?.lastVisitUtc ?? null,
    lastServiceName: customer?.lastServiceName || '',
    appointmentCount: Number(customer?.appointmentCount ?? 0),
...
export function normalizeAccountReport(report: ApiAccountReport | null | undefined): AccountReport {
  ...
  return {
    ...
    categories: (report?.categories ?? []).map((c) => ({
      ...
      soldCount: Number(c?.soldCount ?? 0),
    ...
    customers: (report?.customers ?? []).map((c) => ({
      ...
      customerName: c?.customerName || 'Müşteri',
      accountCount: Number(c?.accountCount ?? 0),
      packageNames: c?.packageNames ?? [],
      installmentCount: Number(c?.installmentCount ?? 0),
      paidInstallmentCount: Number(c?.paidInstallmentCount ?? 0),
      overdueInstallmentCount: Number(c?.overdueInstallmentCount ?? 0),
      ...
      nextDueAmount: Number(c?.nextDueAmount ?? 0),
      sessionsTotal: Number(c?.sessionsTotal ?? 0),
      sessionsUsed: Number(c?.sessionsUsed ?? 0),
      sessionsRemaining: Number(c?.sessionsRemaining ?? 0),
      sellers: mapPackageSellers(c?.sellers),
